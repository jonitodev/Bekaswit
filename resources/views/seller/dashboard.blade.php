        ...
        <div class="col-lg-7">
            <div class="card border-0" style="box-shadow:var(--shadow-sm); border-radius:var(--radius);">
                <div class="card-header bg-transparent border-bottom d-flex justify-content-between align-items-center"
                    ...
                    <a href="{{ route('listing.index') }}" class="small text-decoration-none fw-semibold">Lihat Semua</a>
                </div>
                <div class="card-body p-0">
                                            ...
                                            <td style="padding-left:1.25rem;">
                                                ...
                                                @if($foto)
                                                    <img src="{{ asset('storage/' . $foto->file_path) }}"
                                                         style="width:42px; height:42px; object-fit:cover; border-radius:var(--radius-xs);" alt="">
                                                @else
                                                    <div class="img-placeholder-sm" style="width:42px; height:42px; font-size:1rem;">
                    ...
                    @else
                        <div class="text-center py-4">
                            ...
                            <p class="mt-2 mb-0 small" style="color:var(--text-secondary);">Belum ada barang yang dijual.</p>
                        </div>
                    @endif
                </div>
            </div>
                    ...
                    @if($barangPerKategori->count() > 0)
                        ...
                        @foreach($barangPerKategori as $kat)
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                ...
                                <span class="badge" style="background:var(--primary-light); color:var(--primary);">{{ $kat->barangs_count }}</span>
                            </div>
                            <div class="progress mb-3" style="height:6px; border-radius:3px; background:var(--border-light);">
                                <div class="progress-bar" role="progressbar"
                                     style="width:{{ ($kat->barangs_count / $maxKat) * 100 }}%; background:var(--primary); border-radius:3px;"></div>
                            </div>
                        @endforeach
                    @else
