...
@section('title', 'Edit Barang - Bekaswit')
...
<div class="container py-4">
    <div class="row justify-content-center">
        <div class="col-md-8 col-lg-7">
            ...
            <p class="mb-4" style="color:var(--text-secondary);">Perbarui detail barang Anda</p>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" name="harga" id="harga"
                                       ...
                                       value="{{ old('harga', intval($barang->harga)) }}" min="1000" required>
                                @error('harga')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="kondisi" id="kondisi"
                                ...
                                @foreach(['like-new' => 'Seperti Baru', 'good' => 'Baik', 'fair' => 'Cukup'] as $val => $label)
                                    <option value="{{ $val }}" {{ old('kondisi', $barang->kondisi) === $val ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('kondisi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="photo-preview-container">
                                    @foreach($barang->fotoBarangs as $foto)
                                        <img src="{{ asset('storage/' . $foto->file_path) }}" alt="Foto barang">
                                    @endforeach
                                </div>
                                <small style="color:var(--text-muted); font-size:12px;">Unggah foto baru untuk mengganti semua foto di atas.</small>
                            </div>
                        @endif

                        <div class="mb-4">
                            <label for="fotos" class="form-label">Ganti Foto (Opsional)</label>
                            ...
                            <small style="color:var(--text-muted); font-size:12px;">Maksimal 5 foto. Mengunggah foto baru akan mengganti semua foto lama. Ukuran maks 2MB per foto (JPEG, PNG, WebP)</small>
                            @error('fotos')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            @error('fotos.*')
